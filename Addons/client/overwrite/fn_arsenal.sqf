...
if (true) then
{
	if ((!isRemoteExecuted) && (_mode != "Exit") && (player getVariable ["ECF_var_isDBI", 0]) == 1) exitWith
	{
		["Exit", true] call BIS_fnc_Arsenal;
		hintSilent format ["Für %1 ist das Dabenbank-Interface aktiv.", (name player)];
		[] call Client_fnc_openRoleD;
	};
};
